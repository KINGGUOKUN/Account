...
function ManifestViewModel() {
    ...
    this.getManifest = function () {
        sendAjaxRequest("GET", "http://localhost:1500/api/Manifests",
            {
                start: this.start(),
                end: this.end()
            },
    ...
    this.saveManifest = function () {
        ...
        if (self.currentManifest == null) {
            sendAjaxRequest("POST", "http://localhost:1500/api/Manifests", data,
                function (err, res) {
                    if (err) {
                        alert("添加失败");
                    }
            ...
            sendAjaxRequest("PUT", "http://localhost:1500/api/Manifests", data,
                function (err, res) {
                    if (err) {
                        alert("更新失败" + err);
                    }
    ...
    this.deleteManfest = function (data) {
        sendAjaxRequest("DELETE", "http://localhost:1500/api/Manifests/" + data.ID,
            null,
            function (err, res) {
                if (err) {
                    return alert("删除出错");
